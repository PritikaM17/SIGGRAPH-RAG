...
class RetrievalPipeline:
    ...
    def __init__(self, config: Optional[RetrievalPipelineConfig] = None):
        ...
        # 3. Initialize Qdrant client
        self.qdrant = QdrantClient(url=config.qdrant_url, api_key=config.qdrant_api_key, timeout=120)
        print("✓ Connected to Qdrant")
        
        # 4. Initialize the embedder
        self.embedder = OpenRouterEmbedder(config.openrouter_api_key, config.embedding_model)
        print(f"✓ Embedder initialized ({config.embedding_model})")
        
        # 5. Load chunks from JSON file
        chunks_path = config.chunks_path or "./chunks.json"
